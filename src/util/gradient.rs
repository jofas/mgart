        ...
        match self {
            ...
            Self::Smoothstep { order } => {
                ...
                (0..=*order).fold(0., |acc, n| {
                    ...
                    let p2 = f64::from(Self::pascal_triangle(2 * order + 1, order - n));
...
mod tests {
    ...
    fn discrete_gradient() {
        ...
        assert!(((g.apply(0.25) * 100.).floor() - 33.).abs() <= f64::EPSILON);
        assert!(((g.apply(0.49) * 100.).floor() - 33.).abs() <= f64::EPSILON);
        assert!(((g.apply(0.5) * 100.).floor() - 66.).abs() <= f64::EPSILON);
        assert!(((g.apply(0.74) * 100.).floor() - 66.).abs() <= f64::EPSILON);
        assert!((g.apply(0.75) - 1.).abs() <= f64::EPSILON);
        assert!((g.apply(1.) - 1.).abs() <= f64::EPSILON);
    }
}
