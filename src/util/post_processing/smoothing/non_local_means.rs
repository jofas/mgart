...
#[derive(Serialize, Deserialize, DisplayAsJson, Clone, PartialEq, Debug)]
...
impl SummedAreaTable {
    ...
    fn sum_rectangle_from_center(&self, cx: usize, cy: usize, size_x: usize, size_y: usize) -> f64 {
        let (x0, y0, x1, y1) = discrete_rectangle_from_center(
            cx,
            cy,
            size_x,
...
mod tests {
    ...
    use super::{discrete_rectangle_from_center, NonLocalMeans, SummedAreaTable};
    ...
    #[test]
    fn nlm() {
        let mut frame = Frame::new((1..=16).map(f64::from).collect(), 4, 4);
