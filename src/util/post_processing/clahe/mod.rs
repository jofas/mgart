    ...
    fn tiles(&self, frame: &Frame<f64>) -> Frame<Tile> {
        ...
        for start_block in 0..tiles_h {
            for offset in 0..tiles_w {
                ...
                tiles.push(Tile::new(stride, self.bin_count, self.contrast_limit));
            }
        }
...
impl<'a> InterpolationTiles<'a> {
    ...
    fn interpolation_tiles(&mut self, pos: &Pos, tiles: &'a Frame<Tile>, x: usize, y: usize) {
        let (x1, x2, y1, y2) = match pos {
            Pos::NW => (x.checked_sub(1), Some(x), y.checked_sub(1), Some(y)),
            Pos::NE => (Some(x), x.checked_add(1), y.checked_sub(1), Some(y)),
            Pos::SE => (Some(x), x.checked_add(1), Some(y), y.checked_add(1)),
            Pos::SW => (x.checked_sub(1), Some(x), Some(y), y.checked_add(1)),
    ...
    fn handle_corners_and_borders(&mut self) {
        (self.dn, self.ds, self.dw, self.de) = match self.tiles() {
            ...
            // center
            (Some(_), Some(_), Some(_), Some(_)) => (self.dn, self.ds, self.dw, self.de),
            _ => unreachable!(),
        };
    }
    ...
    fn tiles(&self) -> (Option<&Tile>, Option<&Tile>, Option<&Tile>, Option<&Tile>) {
